...
PUBLIC ASSIGN_FILE_NAMES,INITIALIZE_GLOBAL_DUMPS,INITIALIZE_MESH_DUMPS,WRITE_STATUS_FILES, &
       TIMINGS,FLUSH_GLOBAL_BUFFERS,FLUSH_EVACUATION_BUFFERS,FLUSH_LOCAL_BUFFERS,&
       WRITE_SMOKEVIEW_FILE,DUMP_MESH_OUTPUTS,UPDATE_GLOBAL_OUTPUTS,DUMP_DEVICES,&
	   DUMP_CONTROLS, DUMP_UVW,DUMP_BNDC,DUMP_BNDE
	   
!,DUMP_GEOM,DUMP_GEOM_DIAG
!DUMP_HRR,DUMP_MASS, 
!INITIALIZE_DIAGNOSTIC_FILE,DUMP_RESTART,&
!READ_RESTART,WRITE_DIAGNOSTICS, 
...
INTEGER :: NFM = 0  !!Not Sure whether this is correct??
...
!IF (BTEST(I_EVAC,4)) THEN
!	FN_SMV = TRIM(CHID)//'.smv'
!ELSE
!	FN_SMV = TRIM(CHID)//'_evac.smv'
!ENDIF

IF (I_EVAC<16 .AND. ANY(EVACUATION_ONLY)) THEN
	FN_SMV = TRIM(CHID)//'_evac.smv'
ELSE
	FN_SMV = TRIM(CHID)//'.smv'
ENDIF
      ...
      IF (ABS(AVG_DROP_AREA(II,JJ,KK,LPC%ARRAY_INDEX))>TWO_EPSILON_EB) THEN
         DO N = 1,NUMBER_SPECTRAL_BANDS
            IF (NUMBER_SPECTRAL_BANDS==1) THEN
               BBF = 1._EB
            ELSE
               BBF = 1._EB !BLACKBODY_FRACTION(WL_LOW(N),WL_HIGH(N),RADTMP)
            ENDIF
